...
const M_RequestMetoringCard = ({
  ...
  const { getFieldInstance, validateFields } = form;
  ...
  useEffect(() => {
    ...
    if (cardVisible) {
      ...
      document
        .getElementById(`requestMentoring:${mybook_id}`)
      ...
      actions={[
        ...
        <div
          ...
          onClick={() => {
            if (
              data &&
              data.user_getUserMinInfo.username == mentorIdInputValue
            ) {
              sendRequest();
            }
          }}
                  ...
                  validator: (_, value) =>
                    !data
                      ...
                      : variables.username === value &&
                        data.user_getUserMinInfo.status === "400"
                      ? Promise.reject(
                          new Error("입력하신 아이디는 존재하지 않습니다.")
                        )
                      : Promise.resolve(),
                },
              ]}

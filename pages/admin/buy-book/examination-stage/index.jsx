...
import { MUTATION_CREATE_BUY_BOOK_FROM_CANDIDATE_BOOK } from "../../../../graphql/mutation/buyBook";
...
const ExaminationStageForPreRegistrationBuyBook = () => {
  ...
  const [addBuyBook] = useMutation(
    MUTATION_CREATE_BUY_BOOK_FROM_CANDIDATE_BOOK
  );

  const createBuyBook = async ({ candibook_id, bookTitle, hashtag }) => {
    addBuyBook({
      variables: {
        forCreateBuybook: {
          candibook_id,
      ...
      if (result.data.buybook_createBuybook.status === "200") {
        message.success(`${bookTitle}을 판매책으로 등록 완료하였습니다`, 0.7);
        console.log("판매 등록 성공", result);
      }
    });
  };
              ...
              <table className="w-full min-w-[700px] table-fixed">
                <thead>
                  <tr className="border-collapse border-y border-y-gray-200">
                    ...
                    <th className="text-[1rem] bg-slate-100 w-[10%]">생성</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    data?.candibook_getAllCandibook?.candibooks.map((book) => (
                      ...
                      >
                        ...
                        <td className="text-[1rem] py-[4px] border-r border-collapse border-r-gray-200 text-center">
                          {moment(book.timeCreated).format("YY년M월D일")}
                        </td>

                        <td className="text-[1rem] py-[4px] border-r border-collapse border-r-gray-200 text-center">
                          {book.timeStatusChanged &&
                            moment(book.timeStatusChanged).format("YY년M월D일")}
                        </td>

...
class Container extends Component {
  ...
  saveAnswer = (current_card_info, answer) => {
    console.log(current_card_info, answer);
    const submitted = { cardId: current_card_info._id, answer: answer };
                ...
                {content.card_info.cardtype === "flip" && (
                  <>
                    {content_value.selection !== null &&
                      ...
                      content_value.selection.map((item, index) => (
                        <>
                          <Radio.Group defaultValue={answerField} buttonStyle="solid" size="small" onChange={(e) => this.saveAnswer(content, e.target.value)}>
                            {index === 0 && (
                              <>
                    ...
                    {(content_value.selection === null || content_value.selection.length === 0) &&
                      content_value.face2.map((item, index) => (
                        <>
                          ...
                          <Input type="text" onChange={(e) => this.saveAnswer(content, e.target.value)} defaultValue={answerField} />
                        </>
                      ))}
                  </>
                )}

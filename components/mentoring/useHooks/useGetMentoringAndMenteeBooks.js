...
const useGetMentoringAndMenteeBooks = () => {
  ...
  const getCaculatedData = (mentee, menteeBooks) => {
    const getData = flow(
      ...
      map((mentee) => ({
        ...
        menteeGroupName: _.find(
          mentoringData.mentoring_getMentoring.mentorings[0].mentoring_info
            .menteeGroup,
          { _id: mentee.menteeGroup_id }
        ).name,
        menteeSeq:
          mentoringData.mentoring_getMentoring.mentorings[0].mentoring_info.menteeGroup.findIndex(
            (gr) => gr._id === mentee.menteeGroup_id
          ),
        bookType: _.find(
  ...
  const [getBooksInfo, { data: menteeBooks, error, loading }] = useLazyQuery(
    QUERY_USER_BOOKS_BY_BOOK_IDS,
    {
      onCompleted: (data) => {
        if (data.mybook_getMybookByMybookIDs.status === "200") {
          const fordata = getCaculatedData(
            mentoringData.mentoring_getMentoring.mentorings[0].myMentees,
            data
          );

          setNewData(fordata);
          console.log(
            "멘티정보 업데이트 프로세스 작업 종료(with책 정보 갱신)",
            data
          );
  ...
  const [
    ...
    {
      data: mentoringData,
      previousData: previousMentoringData,
      error: error1,
      loading: loading1,
    },
  ] = useLazyQuery(QUERY_MENTORING, {
    onCompleted: (data) => {
  ...
  useEffect(() => {
    if (mentoringData) {
      ...
      const myMenteeBooksIds = mentoInfo.myMentees.map(
        (mentee) => mentee.mybook_id
      );
      ...
      const previousMentoInfo =
        previousMentoringData.mentoring_getMentoring.mentorings[0];
      const previousMyMenteeBooksIds = previousMentoInfo.myMentees.map(
        (mentee) => mentee.mybook_id
      );
      const isDifferentBetweenDataAndPreviousData = myMenteeBooksIds.some(
        (_value) => !previousMyMenteeBooksIds.includes(_value)
      );

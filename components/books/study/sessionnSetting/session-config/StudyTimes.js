...
const StudyTimes = ({ onOff, studyTimes, changeStudyTimes }) => {
  ...
  return (
    <StyledDivConfigMenuWrapper>
      <Space>
        <InputNumber disabled={!isOn} size="small" min={0} max={studyTimes[1] == null ? 99 : studyTimes[1] - 1} value={studyTimes[0]} onChange={onChangeFrist} />
        ~
        <InputNumber disabled={!isOn} size="small" min={studyTimes[1] == null ? 2 : studyTimes[0] + 1} max={100} value={studyTimes[1]} onChange={onChangeSecond} />
      </Space>
    </StyledDivConfigMenuWrapper>
  );
};

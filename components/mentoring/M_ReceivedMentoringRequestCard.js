...
const M_ReceivedMentoringRequestCard = ({
  mentee,
  declineMentoring,
  menteeGroup,
}) => {
  ...
  const menteeGroupSelector = useMemo(() => {
    return (
      menteeGroup && (
        <>
          <Select
            defaultValue={menteeGroup[0]._id}
            size="small"
            onChange={(v) => (selectorRef.current = v)}
          >
      ...
      <Card.Meta
        ...
        description={
          <>
            ...
            <div>
              요청 날짜: {moment(new Date(Number(reqDate))).format("YY-MM-DD")}
            </div>
            <div>요청메세지:</div>

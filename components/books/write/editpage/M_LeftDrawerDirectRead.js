...
const LeftDrawer = ({ index_changed, indexSets }) => {
  ...
  if (indexinfos) {
    //   console.log("here")
    const bookInfos = JSON.parse(sessionStorage.getItem("books_selected"));
    console.log(bookInfos);
    var treeDatas = indexinfos.map((indexinfo) => {
      console.log(indexinfo);

      const book_title_tmp = bookInfos.filter((item) => item.book_id === indexinfo.indexset_info.mybook_id);
      const book_title = book_title_tmp[0].book_title;
      console.log(book_title_tmp);
      let level_all = [];
  ...
  return (
    <>
      <div onClick={showDrawer} style={{ flexDirection: "column", color: "white" }}>
        ...
        <>
          ...
          >
            {treeDatas.map((treedata) => (
              <>
                <span style={{ fontWeight: "700" }}>{treedata[0].book_title}</span>

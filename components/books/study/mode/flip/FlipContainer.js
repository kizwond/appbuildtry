...
import Popover_CardDetail from "./common/Popover_CardDetail";
...
const Editor = dynamic(
  () => import("../../../../../components/books/write/editpage/Editor"),
  {
    ssr: false,
  }
);
...
const FlipContainer = ({
  ...
  const [indexSelectedForEditor, setIndexSelectedForEditor] =
    useState("default");
  ...
  const [
    ...
    { loading: loading4, error: error4, data: selectedIndexForNewCardAdd },
  ] = useLazyQuery(GetIndex, {
  ...
  function afterGetIndex(data) {
    ...
    sessionStorage.setItem(
      "indexset_id",
      data.indexset_getByMybookids.indexsets[0]._id
    );
  }
  ...
  const [
    cardtypeset_getbymybookids,
    {
      loading: loading5,
      error: error5,
      data: selectedCardTypeSetForNewCardAdd,
    },
  ] = useLazyQuery(GetCardTypeSet, {
    onCompleted: afterGetCardTypeSet,
  });

  function afterGetCardTypeSet(data) {
    console.log(data);
    setCardTypeSetForEditor(
      data.cardtypeset_getbymybookids.cardtypesets[0].cardtypes
    );
    sessionStorage.setItem(
      "cardtypeset_id",
      data.cardtypeset_getbymybookids.cardtypesets[0]._id
    );
  }

  const [
    cardset_getByIndexIDs,
    { loading: loading6, error: error6, data: cardSetGet },
  ] = useLazyQuery(GetCardSet, {
  ...
  function afterGetCardSet(data) {
    ...
    sessionStorage.setItem(
      "cardset_id",
      data.cardset_getByIndexIDs.cardsets[0]._id
    );
  }

  const [cardset_addcardAtSameIndex] = useMutation(AddCard, {
    onCompleted: afteraddcardmutation,
  });
  ...
  function afteraddcardmutation(data) {
    ...
    if (sameIndexCheck === "true") {
      const cardListReceived =
        data.cardset_addcardAtSameIndex.cardsets[0].cards;
      const filteredIndex = cardListReceived.findIndex(
        (item) => item._id === currentCardId
      );
      ...
      const filteredData =
        data.cardset_addcardAtSameIndex.cardsets[0].cards[
          data.cardset_addcardAtSameIndex.cardsets[0].cards.length - 1
        ];
      const {
        mybook_id,
        indexset_id,
  ...
  const speakText = (ttsArray) => {
    ...
    console.log(voiceEn);
    const voiceKo = voices.filter(
      (item) =>
        item.lang === "ko-KR" &&
        item.voiceURI !== "Microsoft Heami - Korean (Korean)"
    );
    console.log(voiceKo);
  ...
  const getTTSData = useCallback(() => {
    ...
    const contents = contentsList.filter((content_value) => {
      if (
        content_value._id === mycontent_id ||
        content_value._id === buycontent_id
      ) {
    ...
    if (contents && currentFace === "front") {
      ...
      contents[0].face1.forEach((c, i) => {
        if (
          ...
          contents[0].face1 !== null &&
          contents[0].face1.length > 0
        ) {
      ...
      if (
        ...
        contents[0].selection !== null &&
        contents[0].selection.length > 0
      ) {
...
class Container extends Component {
  ...
  Diffi5Handler = (diffi, current_card_id, timer) => {
    const now = new Date();
    const card_details_session_origin = JSON.parse(
      sessionStorage.getItem("cardListStudying")
    );
    const current_card_info_index_tmp = card_details_session_origin.findIndex(
      (item) => item.content.mycontent_id === current_card_id
    );
    console.log(current_card_info_index_tmp);
    if (current_card_info_index_tmp === -1) {
      var current_card_info_index = card_details_session_origin.findIndex(
        (item) => item.content.buycontent_id === current_card_id
      );
    } else {
      var current_card_info_index = card_details_session_origin.findIndex(
        (item) => item.content.mycontent_id === current_card_id
    ...
    const current_card_book_id =
      card_details_session_origin[current_card_info_index].card_info.mybook_id;
    const current_card_levelconfig = this.props.levelConfigs.filter(
      (item) => item.levelconfig_info.mybook_id === current_card_book_id
    );

    const card_details_session = calculateStudyStatus(
      null,
      diffi,
      current_card_info_index,
      timer,
      current_card_levelconfig[0]
    );
        ...
        if (this.state.onBackMode) {
          ...
          if (reviewExist_data.length > 0) {
            reviewExist_data.sort(function (a, b) {
              return a.studyStatus.needStudyTimeTmp >
                b.studyStatus.needStudyTimeTmp
                ? 1
                : a.studyStatus.needStudyTimeTmp <
                  b.studyStatus.needStudyTimeTmp
                ? -1
                : 0;
            });
            const earlist_id = reviewExist_data[0]._id;
            const shouldBeSeq = card_details_session.findIndex(
              (item) => item._id == earlist_id
            );
          ...
          //study log 생성
          const current_card_info_tmp = card_details_session.filter(
            (item) => item.content.mycontent_id === current_card_id
          );

          if (current_card_info_tmp.length === 0) {
            var current_card_info = card_details_session.filter(
              (item) => item.content.buycontent_id === current_card_id
            );
          } else {
            var current_card_info = card_details_session.filter(
          ...
          const studyLogCardIds = JSON.parse(
            sessionStorage.getItem("studyLogCardIds")
          );
          if (studyLogCardIds === null) {
            sessionStorage.setItem(
              "studyLogCardIds",
              JSON.stringify([currentCardId])
            );
          } else {
        ...
        if (reviewExist_data.length > 0) {
          reviewExist_data.sort(function (a, b) {
            return a.studyStatus.needStudyTimeTmp >
              b.studyStatus.needStudyTimeTmp
              ? 1
              : a.studyStatus.needStudyTimeTmp < b.studyStatus.needStudyTimeTmp
              ? -1
              : 0;
          });
          const earlist_id = reviewExist_data[0]._id;
  ...
  generateCardSeq = (card_details_session, now, current_card_id) => {
    if (this.state.onBackMode) {
      ...
      if (reviewExist_data.length > 0) {
        reviewExist_data.sort(function (a, b) {
          return a.studyStatus.needStudyTimeTmp > b.studyStatus.needStudyTimeTmp
            ? 1
            : a.studyStatus.needStudyTimeTmp < b.studyStatus.needStudyTimeTmp
            ? -1
            : 0;
  ...
  onClickBeforeCard = () => {
    if (this.state.onBackMode === false) {
      ...
      if (studyLogCardIds === null) {
        ...
        const backModeSeq = studyLogCardIds.findIndex(
          (item) => item === lastCardId[0]
        );
      ...
      const shouldBeSeq = card_details_session.findIndex(
        (item) => item._id === shouldBeBackModeDataId
      );
      console.log(shouldBeSeq);
      if (shouldBeSeq === -1) {
        alert("이전카드 더이상 없어요~");
  ...
  generateBackPassModeStudyStatus = (current_card_id, selection) => {
    ...
    const current_card_info_index = card_details_session_origin.findIndex(
      (item) => item._id === current_card_id
    );
  ...
  generateStudyStatus = (card_details_session, current_card_info_index) => {
    ...
    const getUpdateThis = JSON.parse(
  ...
  hiddenElementTagHandler = (word) => {
    ...
    this.props.cardsetDeleteEffect(
      cardListStudying[currentCardSeq].card_info.cardset_id,
      cardListStudying[currentCardSeq].card_info.card_id,
      "hidden",
      word
    );
  };
  ...
  highlightElementTagHandler = (word) => {
    ...
    // const needToBeChangedIndex = cardListStudying.findIndex((item) => item.card_info.card_id === cardInfo.card_id);
    const newHighlightArray = cardListStudying[
      currentCardSeq
    ].content.highlight.filter((item) => item.targetWord !== word);
          ...
          if (parentId !== null) {
            ...
            parentNode =
              document.getSelection().anchorNode.parentNode.parentNode
                .parentNode.parentNode.parentNode.outerHTML;
            parentNodeInnerHtml =
              document.getSelection().anchorNode.parentNode.parentNode
                .parentNode.parentNode.parentNode.innerHTML;
            var parentId_tmp1 = parentNode.match(/(id=\"\w{10,100}\")/gi);
            var parentId_tmp2 = parentNode.match(
              /(cardSetId\w{10,100}cardId)/gi
            );
            var parentId_tmp3 = parentNode.match(/(cardId\w{10,100})/gi);
            console.log(parentId_tmp1);
            if (parentId_tmp1 !== null) {
            ...
            if (parentId !== null) {
              ...
              parentNode =
                document.getSelection().anchorNode.parentNode.parentNode
                  .parentNode.parentNode.parentNode.parentNode.outerHTML;
              parentNodeInnerHtml =
                document.getSelection().anchorNode.parentNode.parentNode
  ...
  indexSelectOnchange = (value) => {
    ...
    sessionStorage.setItem(
      "selectedCardTypeId",
      this.props.cardTypeSetForEditor[0]._id
    );
  };
  ...
  handleChange = (value) => {
    ...
    if (value[0] !== "default") {
      const hello = this.props.cardTypeSetForEditor.filter(
        (item) => item.cardtype_info.name === value[0]
      );
  ...
  cardTypeInfoNewCard = (cardtype_info, selections) => {
    ...
    if (this.props.cardTypeSetForEditor.length > 0) {
      var cardTypeListNormal = this.props.cardTypeSetForEditor.map(
        (cardType) => {
          return (
            <>
              <Option
                value={[cardType.cardtype_info.name, "normal"]}
    ...
    const editor = (
      <>
        ...
        >
          ...
          <Select
            size="small"
            defaultValue={selectedCardTypeOption}
            style={{ width: 200, fontSize: "0.75rem" }}
            onChange={this.handleChange}
          >
            <Option
              value="default"
              style={{ fontSize: "0.8rem", color: "black", fontWeight: "700" }}
              disabled
            >
              카드타입선택
            </Option>
          ...
          {cardtypeEditor === "flip" && (
            <>
              {selections == undefined && (
                <Button
                  size="small"
                  style={{ fontSize: "0.8rem", marginLeft: "3px" }}
                  onClick={this.addSelections}
                >
                  보기추가
                </Button>
              )}
  ...
  onFinishFromNewCard = (values, from) => {
    ...
    const sameIndexSelectedCheck = sessionStorage.getItem(
      "sameIndexSelectedCheck"
    );
  ...
  render() {
    ...
    if (this.props.levelConfigs) {
      ...
      const resultOfSession = JSON.parse(
        sessionStorage.getItem("resultOfSession")
      );
      var sumClicks = resultOfSession.clicks.total;
      const inserted =
        resultOfSession.numCards.completed.inserted +
        resultOfSession.numCards.hold.inserted +
        resultOfSession.numCards.ing.inserted +
        resultOfSession.numCards.yet.inserted;
      const finished =
        resultOfSession.numCards.completed.finished +
        resultOfSession.numCards.hold.finished +
        resultOfSession.numCards.ing.finished +
        resultOfSession.numCards.yet.finished;
      ...
      const statusCurrent =
        card_details_session[currentSeq].studyStatus.statusCurrent;
      ...
      const current_card_book_id =
        card_details_session[currentSeq].card_info.mybook_id;
      const current_card_id_tmp =
        card_details_session[currentSeq].content.mycontent_id;
      if (current_card_id_tmp === null) {
        var current_card_id =
          card_details_session[currentSeq].content.buycontent_id;
      } else {
        var current_card_id =
          card_details_session[currentSeq].content.mycontent_id;
      }
        ...
        var diffiButtons = (
          <>
            <Button
              ...
              size="small"
              style={{ fontSize: "1rem" }}
              onClick={() => this.onClickRestoreHandler(current_card_id)}
              type="primary"
            >
              복원
            </Button>
        ...
        var diffiButtons = (
          <div
            style={{
              ...
              borderRadius: "5px",
              background:
                "linear-gradient(270deg, rgba(65,255,0,1) 0%, rgba(232,255,0,1) 50%, rgba(255,0,0,1) 100%)",
              textAlign: "left",
              height: "33.41px",
            }}
          >
            ...
            >
              <button
                ...
                onClick={() =>
                  this.onDiffClickHandler(current_card_id, this.state.time, 2.5)
                }
                style={{ width: "5%" }}
              ></button>
              <button
                className="diffi_button"
                onClick={() =>
                  this.onDiffClickHandler(current_card_id, this.state.time, 7.5)
                }
                style={{ width: "5%" }}
              ></button>
              <button
              ...
              <button
                className="diffi_button"
                onClick={() =>
                  this.onDiffClickHandler(current_card_id, this.state.time, 95)
                }
                style={{
                  width: "25%",
                  fontSize: "0.8rem",
              ...
              >
                ...
                <span>
                  <CalculateIf
                    ...
                    levelConfigs={current_card_levelconfig[0]}
                  />
                </span>
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 12.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 17.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 22.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 27.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 32.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 37.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 42.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 47.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 52.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 57.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 62.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 67.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 72.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 77.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 82.5)
              }
              style={{ width: "5%" }}
            ></button>
            <button
              className="diffi_button"
              onClick={() =>
                this.onDiffClickHandler(current_card_id, this.state.time, 87.5)
              }
              style={{ width: "5%" }}
            ></button>
            <span
      ...
      var goBackToCurrent = (
        <>
          <Button
            ...
            type="primary"
            style={{ fontSize: "0.8rem" }}
            onClick={this.onClickGoBackToOrigin}
          >
            원위치에서 학습 이어하기
          </Button>
        </>
      ...
      var moreMenu = (
        <>
          <Popover
            visible={this.state.popoverClicked}
            onVisibleChange={this.handleClickPopover}
            placement="left"
            content={moreMenuContents}
            trigger="click"
          >
            <Button
              icon={<DashOutlined />}
      ...
      var cardInfoStatusBar = (
        <>
          <span>
            ...
            Lv.
            {Math.round(
              card_details_session[currentSeq].studyStatus.levelCurrent * 10
            ) / 10}
          </span>
        </>
      );
            ...
            <>
              <Space>
                ...
                <Button
                  ...
                  size="small"
                  style={{ fontSize: "1rem" }}
                  onClick={() => this.onClickHoldHandler(current_card_id)}
                  type="primary"
                >
                  보류
                </Button>
                <Button
                  ...
                  size="small"
                  style={{ fontSize: "1rem" }}
                  onClick={() => this.onClickCompletedHandler(current_card_id)}
                  type="primary"
                >
                  졸업
                </Button>
                  ...
                  {!this.state.updateMemoState && (
                    <>
                      ...
                      <div>
                        <Button
                          size="small"
                          onClick={() => this.updateMemo(content.content.memo)}
                        >
                          수정
                        </Button>
                      </div>
                  ...
                  {this.state.updateMemoState && (
                    <>
                      <Input
                        ...
                        onChange={(e) => this.addMemo(e)}
                      />
                      <Button
                        size="small"
                        onClick={() => this.saveMemo(content.card_info)}
                      >
                        저장
                      </Button>
                    </>
                  ...
                  content.content.hidden.map((item) => {
                    return (
                      <>
                        <Tag
                          onClick={() =>
                            this.hiddenElementTagHandler(item.targetWord)
                          }
                        >
                          <div
                            style={{
                  ...
                  content.content.underline.map((item) => {
                    return (
                      <>
                        <Tag
                          onClick={() =>
                            this.underlineElementTagHandler(item.targetWord)
                          }
                        >
                          <div
                            style={{
                  ...
                  content.content.highlight.map((item) => {
                    return (
                      <>
                        <Tag
                          onClick={() =>
                            this.highlightElementTagHandler(item.targetWord)
                          }
                        >
                          <div
                            style={{
                ...
                >
                  ...
                  >
                    <div>
                      <Select
                        size="small"
                        value={this.props.bookSelectedForEditor}
                        style={{ width: 200, fontSize: "1rem" }}
                        onChange={this.bookSelectOnchange}
                      >
                        <Option
                          value="default"
                          style={{
                    ...
                    <div>
                      <Select
                        size="small"
                        value={this.props.indexSelectedForEditor}
                        style={{ width: 200, fontSize: "1rem" }}
                        onChange={this.indexSelectOnchange}
                      >
                        <Option
                          value="default"
                          style={{
                      ...
                      {content.card_info.index_id ===
                        this.props.indexSelectedForEditor && (
                        <>
                          <div>
                            <Checkbox onChange={this.sameIndexSelected}>
                              해당카드 바로뒤에 저장
                            </Checkbox>
                          </div>
                        </>
                    ...
                    <div>
                      {this.props.indexSelectedForEditor !== "default" &&
                        this.props.newCardEditor}
                    </div>
                  </Modal>
      ...
      var makerFlagContent = card_details_session.map((content, index) => {
        ...
        const show_contents = contentsList.map((content_value) => {
          if (
            ...
            const comment_font_underline =
              makerFlagStyle.comment_font.underline;
              ...
              <>
                <div
                  style={{
                    ...
                    paddingBottom:
                      makerFlagStyle.row_style.inner_padding.bottom,
                            ...
                            <>
                              ...
                              >
                                <Alter
                                  ...
                                  item={item}
                                  index={index}
                                  getSelectionText2={this.getSelectionText2}
                                  cardTypeSets={cardTypeSets}
                                />
                              ...
                              <>
                                <div
                                  ...
                                  style={{
                                    ...
                                    textAlign:
                                      row_font.face1[row_font.face1.length - 1]
                                        .align,
                                    fontWeight: `${
                                      row_font.face1[row_font.face1.length - 1]
                                        .bold === "on"
                                  ...
                                  style={{
                                    ...
                                    marginBottom:
                                      row_style.face2[index].outer_margin
                                        .bottom,
                                    marginLeft:
                                      row_style.face2[index].outer_margin.left,
                                    marginRight:
                                      row_style.face2[index].outer_margin.right,
                                    paddingTop:
                                      row_style.face2[index].inner_padding.top,
                                    paddingBottom:
                                      row_style.face2[index].inner_padding
                                        .bottom,
                                    paddingLeft:
                                      row_style.face2[index].inner_padding.left,
                                    paddingRight:
                                      row_style.face2[index].inner_padding
                                        .right,
        ...
        >
          ...
          <div style={style_study_layout_bottom}>
            <div style={{ width: "100%", height: "100%" }}>
              <div style={{ height: "15px", paddingLeft: "5px" }}>
                {makerFlagContent}
              </div>
              <div style={contentsDisplay}>
                ...
                {!this.state.flip && (
                  <div style={{ height: "100%" }}>{face2Contents}</div>
                )}
              </div>
            </div>
          </div>
...
const CalculateIf = ({ currentSeq, levelConfigs }) => {
  const estimate = calculateStudyStatus(
    null,
    "prediction",
    currentSeq,
    null,
...
const Alter = ({ content, item, index, getSelectionText2, cardTypeSets }) => {
  ...
  if (content.content.hidden.length > 0) {
    content.content.hidden.map((element) => {
      ...
      altered = altered.replaceAll(
        remake9,
        `<span style="background-color:${color}; color:${color}">${element.targetWord}</span>`
      );
    });
  }
  if (content.content.underline.length > 0) {
    content.content.underline.map((element) => {
      ...
      const thickness =
        cardTypeSets[0].studyTool.underline[element.toolType].attr1;
      const lineType =
        cardTypeSets[0].studyTool.underline[element.toolType].attr2;
      const tmp = element.targetWord;
      const remake = tmp.replaceAll("'", "&#39;");
      const remake2 = remake.replaceAll(">", "&gt;");
      const remake3 = remake2.replaceAll("<", "&lt;");
      ...
      altered = altered.replaceAll(
        remake9,
        `<span style="display:inline-block; border-bottom: ${thickness}px ${lineType} ${color}">${element.targetWord}</span>`
      );
    });
  }
  ...
  if (varUA.indexOf("android") > -1) {
    ...
    var contentsToRender = (
      <>
        <div
          id={`${content._id}face1row${index + 1}cardSetId${
            content.card_info.cardset_id

...
import { BOOK_CHANGE_CATEGORY_MUTATION, GET_My_BOOK_CATEGORY_AND_MY_BOOK_CATEGORIES } from '../../../graphql/query/studyCategory';
import { Select, Button, Alert, Space } from 'antd';

const CategorySetting = ({ book_id }) => {
  const [categories, setCategories] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('');
  const { loading, error, data } = useQuery(GET_My_BOOK_CATEGORY_AND_MY_BOOK_CATEGORIES, {
    variables: { mybook_id: book_id },
    onCompleted: (_data) => {
      setCategories(_data.mybookcate_get.mybookcates);
      setSelectedCategory(_data.mybook_getbyMybookid.mybooks[0].mybook_info.mybookcate_id);
  ...
  return (
    <Alert
      message={
        data && (
          <>
            <b>{data && data.mybook_getbyMybookid.mybooks[0].mybook_info.title}</b>이 이동할 카테고리를 선택하십시오.
          </>
        )
      }
      description={
        <>
          ...
          <Space align="center">
            <Select value={selectedCategory} style={{ width: 120 }} onChange={(v) => setSelectedCategory(v)} placeholder="카테고리를 선택해 주세요.">
              {categories && categories.map((category) => <Select.Option key={category._id}>{category.mybookcate_info.name}</Select.Option>)}
            </Select>
            <span>카테고리로</span>
